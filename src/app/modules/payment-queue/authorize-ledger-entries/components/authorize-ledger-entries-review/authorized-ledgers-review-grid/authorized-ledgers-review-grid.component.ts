...
@Component({
  selector: 'app-authorized-ledgers-review-grid',
  templateUrl: './authorized-ledgers-review-grid.component.html'
})
export class AuthorizedLedgersReviewGridComponent extends ListView implements OnDestroy, OnChanges {
  public readonly gridId: GridId = GridId.ReviewAuthorizeLedgerEntries;
  ...
  public fetchGridData(gridParams: IServerSideGetRowsParamsExtended, batchActionId: number): void {
    ...
    searchOpts.filterModel = [
      ...searchOpts.filterModel,
      new FilterModel({ filter: 1, type: 'equals', key: 'authorizedLedgerEntriesReviewFilter', filterType: FilterTypes.Number }),
      new FilterModel({ filter: batchActionId, type: 'equals', key: 'batchActionId', filterType: FilterTypes.Number }),
    ];

    this.store.dispatch(paymentQueueActions.GetSelectedPaymentQueueAuthorizedLedgersGrid({ searchOpts, agGridParams: gridParams, projectId: this.projectId }));
  }
