...
@Component({
  selector: 'app-authorize-ledger-entries-result-grid',
  templateUrl: './authorize-ledger-entries-result-grid.component.html'
})
export class AuthorizeLedgerEntriesResultGridComponent extends ListView implements OnDestroy, OnChanges {
  public readonly gridId: GridId = GridId.ResultAuthorizeLedgerEntries;
  ...
  public fetchGridData(gridParams: IServerSideGetRowsParamsExtended, batchActionId:number): void {
      ...
      searchOpts.filterModel = [
        ...searchOpts.filterModel,
        new FilterModel({ filter: 1, type: 'equals', key: 'authorizedLedgerEntriesResultFilter', filterType: FilterTypes.Number }),
        new FilterModel({ filter: batchActionId, type: 'equals', key: 'batchActionId', filterType: FilterTypes.Number }),
      ];

      this.store.dispatch(paymentQueueActions.GetSelectedPaymentQueueAuthorizedLedgersResultGrid({ searchOpts, agGridParams: gridParams, projectId: this.projectId }));
  }
