...
import { Payment, PaymentItemDetail } from '@app/models';
...
@Component({
  selector: 'app-payment-item-detail-list',
  templateUrl: './payment-item-detail-list.component.html',
  styleUrls: ['./payment-item-detail-list.component.scss'],
})
export class PaymentItemDetailListComponent extends ListView implements OnInit {
  readonly gridId: GridId = GridId.PaymentItemDetailList;
  public payment: Payment;
  public entityType: EntityTypeEnum = EntityTypeEnum.PaymentItem;
  ...
  protected fetchData(gridParams: IServerSideGetRowsParamsExtended): void {
    ...
    if (this.payment) {
      if (this.payment.hasTransferItems) {
        this.store.dispatch(paymentActions.GetTransferItemDetails({ params: gridParams, transferId: +this.payment.id }));
      } else {
        this.store.dispatch(paymentActions.GetPaymentItemDetails({ params: gridParams, paymentId: +this.payment.id }));
      }
    } else {
      this.payment$.pipe(
        takeUntil(this.ngUnsubscribe$),
        ...
        if (this.payment.hasTransferItems) {
          ...
          this.store.dispatch(paymentActions.GetPaymentItemDetails({ params: gridParams, paymentId: +data.id }));
        }
      });
    }
  }
  ...
  public onRowDoubleClicked(params): void {
    const paymentItemDetail = params.data as PaymentItemDetail;
    if (paymentItemDetail.clientId && paymentItemDetail.claimSettlementLedgerId) {
      const navSettings = AGGridHelper.getNavSettings(this.getGridApi());
      this.store.dispatch(
        CreatePager({
          relatedPage: RelatedPage.PaymentItemsDetails,
          settings: navSettings,
          pager: { payload: { paymentId: paymentItemDetail.paymentId } as claimantActions.IClaimantSummaryPagerPayload },
        }),
      );
      ...
      this.store.dispatch(claimantActions.GotoLedgerDetails({ claimantId: paymentItemDetail.clientId, ledgerId: paymentItemDetail.claimSettlementLedgerId }));
    } else if (paymentItemDetail.clientId) {
      const navSettings = AGGridHelper.getNavSettings(this.getGridApi());
      this.store.dispatch(
        CreatePager({
          relatedPage: RelatedPage.PaymentItemsDetails,
      ...
      const claimantDetailsRequest: ClaimantDetailsRequest = { gridParamsRequest: this.gridParams.request };
      this.store.dispatch(claimantActions.SetClaimantDetailsRequest({ claimantDetailsRequest }));
      this.store.dispatch(claimantActions.GoToClaimantLedgerSummary({ claimantId: paymentItemDetail.clientId }));
    }
  }
