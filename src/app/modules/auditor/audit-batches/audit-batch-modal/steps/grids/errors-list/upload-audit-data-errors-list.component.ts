...
@Component({
  selector: 'app-upload-audit-data-errors-list',
  templateUrl: './upload-audit-data-errors-list.component.html',
})
export class UploadAuditDataErrorsListComponent extends ListView {
  @Input() public auditRunId: number;

  public gridId: GridId = GridId.AuditBatchReviewErrors;
  ...
  private addDataValidationResultIdFilterIntoSearchParams(params: IServerSideGetRowsParamsExtended): void {
    params.request.filterModel.push(new FilterModel({
      filter: AuditBatchUploading.CompletedWithErrors,
      filterType: FilterTypes.Number,
      key: 'previewStatusId',
      type: SearchConditionsEnum.Equals,
    }));
