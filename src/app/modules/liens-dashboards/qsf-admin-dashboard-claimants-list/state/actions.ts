...
const featureName = '[QSF Admin Dashboard Claimants]';
export const gridFeatureName = `${featureName} [Grid]`;

export const GetClaimantsList = createAction(`${featureName} Get Claimants List`, props<{ projectId: number, rootProductCategoryId: number, agGridParams?: IServerSideGetRowsParamsExtended, lienTypes?: number[], lienPhases?: number[], clientStages?: number[], bypassSpinner?: boolean }>());
...
export const DownloadClaimants = createAction(`${featureName} Download Claimants List`, props<{ channelName: string, agGridParams: IServerSideGetRowsParamsExtended, columns: ColumnExport[], rootProductCategoryId: number, projectId: number }>());
