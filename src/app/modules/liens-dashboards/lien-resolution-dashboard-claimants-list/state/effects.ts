...
import * as fromLienResolutionDashboard from '../../lien-resolution-dashboard/state/selectors';
import { LienState } from '../../state/reducer';

@Injectable()
export class LienResolutionDashboardClaimantsListEffects {
  public project$ = this.store$.select(fromLienResolutionDashboard.projectDetails);
  public searchParams$ = this.store$.select(selectors.searchParams);
  public selectedStages$ = this.store$.select(fromLienResolutionDashboard.selectedStages);
  public selectedTypes$ = this.store$.select(fromLienResolutionDashboard.selectedTypes);
  public selectedPhases$ = this.store$.select(fromLienResolutionDashboard.selectedPhases);
  ...
  getClaimantsList$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(([action, searchParams, selectedStages, selectedTypes, selectedPhases]) => {
      ...
      return this.projectsService.getClientsByLienResolution(
  ...
  gotoClaimantDetails$ = createEffect(() => this.actions$.pipe(
    ...
    tap(action => this.router.navigate(
      [`claimants/${action.claimantDetailsRequest.id}/services/${action.claimantDetailsRequest.rootProductCategoryId}`],
      {
        state: {
