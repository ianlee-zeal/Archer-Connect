...
@Injectable()
export class ReleaseDashboardEffects {
  constructor(
  ...
  GetReleaseInGoodOrderSummary$ = createEffect((): any => this.actions$.pipe(
    ofType(actions.GetReleaseInGoodOrderSummary),
    mergeMap(action =>
      this.projectsService.GetReleaseInGoodOrderSummary(
        action.rootProductCategoryId,
        action.projectId,
        action.lienPhases,
        action.clientStages)
        .pipe(
          switchMap(response => [actions.GetReleaseInGoodOrderSummarySuccess({ summary: new ReleaseInGoodOrderSummary(response) })]),
          catchError(error => of(actions.Error({ errorMessage: error }))),
        )),
  ));
