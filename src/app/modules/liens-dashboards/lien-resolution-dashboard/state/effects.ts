...
@Injectable()
export class LienResolutionDashboardEffects {
  constructor(
  ...
  getPhasesSummary$ = createEffect((): any => this.actions$.pipe(
    ...
    mergeMap(action => this.projectsService.GetProductPhasesSummary(
      action.rootProductCategoryId,
      action.projectId,
      action.lienTypes,
      action.lienPhases,
  ...
  getTypesSummary$ = createEffect((): any => this.actions$.pipe(
    ...
    mergeMap(action => this.projectsService.GetProductTypesSummary(
      action.rootProductCategoryId,
      action.projectId,
      action.lienPhases,
      action.clientStages,
  ...
  back$ = createEffect(() => this.actions$.pipe(
    ofType(actions.Back),
    tap(action => this.router.navigate(['projects', action.projectId])),
  ), { dispatch: false });
